    ...
    return (value, path) => {
        ...
        for (const v of vals) {
            if (typeof v === 'object' && v !== null)
                continue; // nested object (e.g. ImageObject), skip
            if (!isUrl(v)) {
...
function positiveNumberValidator(field) {
    return (value, path) => {
        if (value == null)
            return null;
...
export const SCHEMA_TYPE_MAP = new Map(SCHEMA_TYPES.map((def) => [def.type.toLowerCase(), def]));
const SUSPICIOUS_PATTERNS = ['app', 'seo', 'tapita', 'booster', 'plugin', 'extension', 'theme'];
export function isSuspiciousType(type) {
